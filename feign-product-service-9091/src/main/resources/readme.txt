...
1.
cd 到指定專案路徑下
例如 windows: cd C:\Users\liaojames\workspace-microservice-20240602\feign-product-service-9091
例如 mac: cd /Users/liaojames/workspace-microservice-20240602/feign-product-service-9091
...
透過 sts4/eclipse 來打包
feign-product-service -> Run As -> Maven build -> Goals 輸入 "clean package" -> 按下 Run

3.
執行 jar
java -jar target/打包名稱.jar
例如: java -jar target/feign-product-9091-0.0.1-SNAPSHOT.jar

java -jar C:\Users\liaojames\workspace-microservice-20240602\feign-product-service-9091\target\feign-product-service-9091-0.0.1-SNAPSHOT.jar
...
2.構建 Docker 影像：
首先，確保您已經在專案根目錄下（包含 Dockerfile 的位置）成功執行了 Maven 打包命令 mvn clean package，並且 target 目錄中生成了 feign-product-service-0.0.1-SNAPSHOT.jar 文件。
然後，在包含 Dockerfile 的目錄下運行以下命令來構建 Docker 鏡像：

docker build -t feign-product-service . 
	-t 是 tag 的意思, 這裡是給這個鏡像取了一個名字叫 feign-product-service
	注意最後有一個點, 表示當前目錄
